...
export const formatDateForExternalAPI = (date = new Date(), formatStr = "yyyy-MM-dd HH:mm:ss") => {
  ...
  try {
    ...
    console.log(`[TZ DEBUG] Fecha generada para API externa: "${formattedDate}" (Zona: ${tz})`);

    return formattedDate;
  } catch (e) {
    console.warn(`⚠️ [TZ ERROR] Zona horaria inválida '${tz}', usando UTC. Error: ${e.message}`);

    // Usa UTC como fallback
    const formattedDate = format(date, formatStr, { timeZone: "UTC" });

    console.log(`[TZ DEBUG] Fecha generada en fallback (UTC): "${formattedDate}"`);
